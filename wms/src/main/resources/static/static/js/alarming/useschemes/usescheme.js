layui.use(['table','form', 'jquery', 'laydate', 'layer', 'laypage', 'dialog', 'element', 'upload', 'layedit'], function() {
                ...
                if (result.code == "9") {
                    ...
                    for (var i = 0; i < length; i++){
                        ...
                        var divhtml = "<form class=\"layui-form\">\n" +
                            ...
                            "                        <button class=\"layui-btn layui-btn-xs layui-btn-warm schemeLineBtn\" onclick='closePlan(\""+divData.id+"\",\""+divData.alarmingId+"\",\""+divData.schemeId+"\");return false;' lay-filter='inEquipment' lay-event=\"edit\"><i class=\"layui-icon\" style=\"color: #323232;\">&#xe651;</i></button>\n" +
    ...
    window.infoScheme = function (planId,alarmingId,schemeId) {
        if (alarmingId == null || alarmingId == ""){
            ...
            var url="/wms/toInfoPlan?planId="+planId+"&alarmingId="+alarmingId+"&schemeId="+schemeId+"&operation=info";
    ...
    window.startPlan = function (planId,alarmingId,schemeId) {
        if (alarmingId == null || alarmingId == ""){
            ...
            var url="/wms/toStartPlan?planId="+planId+"&alarmingId="+alarmingId+"&schemeId="+schemeId+"&operation=start";
            parent.page("预案详情", url, iframeObj, w = "95%", h = "90%");
            return false;
        }
    }
    ...
    //预案关闭
    window.closePlan = function (planId,alarmingId,schemeId) {
        if (alarmingId == null || alarmingId == ""){
            alert("无法查阅预案详情，未找到该警情主键标识;");
            return false;
        }else if (planId == null || planId == ""){
            ...
            var url="/wms/toClosePlan?planId="+planId+"&alarmingId="+alarmingId+"&schemeId="+schemeId+"&operation=close";
            parent.page("预案详情", url, iframeObj, w = "95%", h = "90%");
            return false;
        }
    }
